...
import cn.zhangchuangla.medicine.admin.service.MallProductTagRelService;
...
public class MallProductTagServiceImpl extends ServiceImpl<MallProductTagMapper, MallProductTag>
    ...
    public Page<MallProductTagAdminVo> listTags(MallProductTagListQueryRequest request) {
        ...
        Long filterTypeId = resolveFilterTypeId(request.getTypeId(), request.getTypeCode());
        if (filterTypeId != null && filterTypeId < 0) {
            return emptyPage(request);
        }
        LambdaQueryWrapper<MallProductTag> queryWrapper = new LambdaQueryWrapper<>();
        if (StringUtils.hasText(request.getName())) {
            queryWrapper.like(MallProductTag::getName, request.getName().trim());
        }
        if (filterTypeId != null) {
            queryWrapper.eq(MallProductTag::getTypeId, filterTypeId);
        }
        if (request.getStatus() != null) {
            queryWrapper.eq(MallProductTag::getStatus, request.getStatus());
        }
        queryWrapper.orderByAsc(MallProductTag::getTypeId)
                .orderByAsc(MallProductTag::getSort)
                .orderByDesc(MallProductTag::getId);
        Page<MallProductTag> tagPage = page(request.toPage(), queryWrapper);
        Map<Long, MallProductTagType> typeMap = mallProductTagTypeService.listTypeEntityMapByIds(
                tagPage.getRecords().stream().map(MallProductTag::getTypeId).filter(Objects::nonNull).distinct().toList()
        );
        List<MallProductTagAdminVo> rows = sortTags(tagPage.getRecords(), typeMap).stream()
                .map(tag -> toAdminVo(tag, typeMap.get(tag.getTypeId())))
                .toList();
        Page<MallProductTagAdminVo> result = new Page<>(tagPage.getCurrent(), tagPage.getSize(), tagPage.getTotal());
    ...
    public boolean addTag(MallProductTagAddRequest request) {
        ...
        Integer normalizedStatus = normalizeStatus(request.getStatus());
        assertTagNameUnique(null, type.getId(), normalizedName);

        MallProductTag tag = new MallProductTag();
        tag.setName(normalizedName);
        tag.setTypeId(type.getId());
        tag.setSort(normalizeSort(request.getSort()));
        tag.setStatus(normalizedStatus);
    ...
    private List<MallProductTagFilterGroup> buildFilterGroups(List<Long> orderedTagIds,
        ...
        for (Long tagId : orderedTagIds) {
            ...
            MallProductTagFilterGroup group = groupMap.computeIfAbsent(type.getId(), key -> {
                MallProductTagFilterGroup filterGroup = new MallProductTagFilterGroup();
     ...
     * @param typeId   标签类型ID
     * @param typeCode 标签类型编码
     * @return 解析后的标签类型ID，不存在时返回 -1
     */
    private Long resolveFilterTypeId(Long typeId, String typeCode) {
        if (!StringUtils.hasText(typeCode)) {
            return typeId;
        }
        MallProductTagType type = mallProductTagTypeService.getTypeEntityByCode(typeCode);
        ...
        if (typeId != null && !Objects.equals(typeId, type.getId())) {
            return -1L;
        }
        return type.getId();
    }
