 ...
 * 测试接口：
 * <ul>
 *     <li>GET /agent/admin/product/search - 商品搜索占位接口</li>
...
class AgentProductControllerTests {
     ...
     * 测试商品搜索占位接口返回成功。
     * <p>
     * 测试目的：验证商品搜索占位接口能够正常返回成功响应，
     * 该接口目前为智能体工具调用的占位接口。
     * 测试接口：GET /agent/admin/product/search
     * 预期结果：返回状态码 200，数据为 null
     */
    @Test
    void searchProduct_ShouldReturnSuccess() {
        var result = controller.searchProduct();
